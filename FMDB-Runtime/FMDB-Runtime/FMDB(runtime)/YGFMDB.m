...
static YGFMDB *DBManager = nil;

@implementation YGFMDB
{
...
- (void (^)(__weak Class cls, NSString *tbName, NSString *pk))create_model
{
    return ^(Class cls, NSString *tbName, NSString *pk){
        ...
        NSArray *types = dic[yg_PROPERTY_YTPE];
        NSArray *names = dic[yg_PROPERTY_NAME];
        
        // 语句
        NSString *sql = ygFORMAT(@"%@ %@", yg_SQL_CREATE, tbName);
        ...
        for (int i = 0; i < types.count; i++)
        {
            if (0 == i && (i != types.count - 1))
            {
                if ([pk isEqualToString:names[i]])
                    sql = [sql stringByAppendingFormat:@"(%@ %@ %@,", names[i], types[i], yg_SQL_PK];
                else
...
//        NSLog(@"%@", sql);
        ...
        [_dbQueue inDatabase:^(FMDatabase *db) {
            ...
            {
                NSLog(@"YGFMDB 插入 %@ 表中的数据失败!", sqls[0]);
            }
        }];
    };
}
...
- (void (^)(__weak id model, NSString *tbName))insert_model
{
    return ^(id model, NSString *tbName){
        ...
        NSArray *property = [self getClassProperty:[model class]][yg_PROPERTY_NAME];
        
        NSMutableArray *values = [NSMutableArray new];
        
        NSString *sql = ygFORMAT(@"%@ %@", yg_SQL_INSERT, tbName);
...
//        NSLog(@"%@", sql);
        ...
        [_dbQueue inDatabase:^(FMDatabase *db) {
            ...
            if (![db executeUpdate:sql withArgumentsInArray:values])
            {
                NSLog(@"YGFMDB 插入 %@ 表中的数据失败!", tbName);
            }
        }];
    };
}
...
- (NSArray * (^)(NSString *tableName, __weak Class cls))findAll
{
    return ^(NSString *tableName, __weak Class cls) {
        ...
        [_dbQueue inDatabase:^(FMDatabase *db) {
            ...
            // 根据表名查询数据库
            NSString *sql = ygFORMAT(@"%@ %@", yg_SQL_SELECT_ALL, tableName);
...
- (NSArray *)tableFields:(NSString *)tableName
{
    ...
    [_dbQueue inDatabase:^(FMDatabase *db) {
        ...
        // 查询某个表的所有字段名和信息
        NSString *sql = ygFORMAT(@"%@(%@)", yg_SQL_PRAGMA, tableName);
...
//        NSLog(@"%@ -- %@", propertyName, propertyType);
    ...
    return  [NSDictionary dictionaryWithObjectsAndKeys:propertyNames,yg_PROPERTY_NAME,propertyTypes,yg_PROPERTY_YTPE, nil];
}
